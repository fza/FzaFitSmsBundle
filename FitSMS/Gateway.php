...
namespace fza\FitSMSGatewayBundle\FitSMS;

use fza\FitSMSGatewayBundle\Helper\NumberHelper;
use fza\FitSMSGatewayBundle\SMS;
...
class Gateway
{
    ...
    public function setOptions( array $options )
    {
        ...
        if( $isInvalid )
        {
            throw new \InvalidArgumentException( sprintf( 'The FitSMSGateway does not support the following options: "%s".', implode( '\', \'', $invalid ) ) );
        }
    }
    ...
    public function setOption( $key, $value )
    {
        if( !array_key_exists( $key, $this->options ) )
        {
            throw new \InvalidArgumentException( sprintf( 'The FitSMSGateway does not support the "%s" option.', $key ) );
        }
    ...
    public function sendSMS( SMS $sms, \DateTime $time = null )
    {
        ...
        else
        {
            if( null !== $this->logger )
            {
                $this->logger->warn( sprintf( 'Failed connecting to the FitSMS gateway server at "\s".', $this->options['gateway_uri'] ) );
            }
        ...
        if( null !== $response )
        {
            if( $response->isFailure() )
            {
                if( null !== $this->logger )
                {
                    $this->logger->err( sprintf( 'Failed sending SMS\s. (\s)', null !== $messageId ? ', message ID: ' . $messageId : '', $response->getMessage() ) );
                }
            ...
            if( null !== $this->logger )
            {
                $this->logger->info( sprintf( 'SMS sent successfully\s\s. (\s)', $response->isTest() ? ' in test mode' : '', null !== $messageId ? ', message ID: ' . $messageId : '', $response->getMessage() ) );
            }
        }
